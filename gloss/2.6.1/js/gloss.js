/*! Gloss 2.6.1 | https://utahhealthcare.github.io/gloss-docs/ | (c) 2019 - 2022 University of Utah Health | BSD-3-Clause License */
...
(function (global, factory) {
    ...
    var parseCssVar = memoize(function (name) {
        /* usage in css: .gls-name:before { content:"xyz" } */

        var element = append(document.documentElement, fragment('<div>'));

        addClass(element, ("gls-" + name));
    ...
    function getCssVar(name) {
        return (isIE
            ? parseCssVar(name)
            : getStyles(document.documentElement).getPropertyValue(("--gls-" + name))
    ...
    function transition(element, props, duration, timing) {
        ...
        return Promise$1.all(toNodes(element).map(function (element) { return new Promise$1(function (resolve, reject) {
                ...
                once(element, 'transitionend transitioncanceled', function (ref) {
                    ...
                    clearTimeout(timer);
                    removeClass(element, 'gls-transition');
                    css(element, {
                        transitionProperty: '',
                        transitionDuration: '',
                ...
                addClass(element, 'gls-transition');
                css(element, assign({
    ...
    var animationPrefix = 'gls-animation-';
    ...
    function animate$1(element, animation, duration, origin, out) {
        ...
        return Promise$1.all(toNodes(element).map(function (element) { return new Promise$1(function (resolve, reject) {
                ...
                if (startsWith(animation, animationPrefix)) {
                    origin && addClass(element, ("gls-transform-origin-" + origin));
    ...
    */
    ...
    function globalAPI (Gloss) {
        ...
        Gloss.extend = function (options) {
            ...
            var Super = this;
            var Sub = function GlossComponent(options) {
                this._init(options);
            };
    ...
    function hooksAPI (Gloss) {
        ...
        Gloss.prototype._callUpdate = function (e) {
            var this$1$1 = this;
    ...
    function instanceAPI (Gloss) {
        ...
        Gloss.prototype.$destroy = function (removeEl) {
        ...
        Gloss.prototype.$update = function (element, e) {
        ...
        var componentName = memoize(function (name) { return Gloss.prefix + hyphenate(name); });
    ...
    function componentAPI (Gloss) {
        ...
        Gloss.component = function (name, options) {
            ...
            Gloss[name] = function (element, data) {
                ...
                var component = Gloss.component(name);
                ...
                function init(element) {
                    ...
                    var instance = Gloss.getComponent(element, name);
            ...
            if (opt.install) {
                opt.install(Gloss, opt, name);
            }

            if (Gloss._initialized && !opt.functional) {
                fastdom.read(function () { return Gloss[name](("[gls-" + id + "],[data-gls-" + id + "]")); });
            }

            return components[name] = isPlainObject(options) ? opt : options;
        };

        Gloss.getComponents = function (element) { return element && element[DATA] || {}; };
        Gloss.getComponent = function (element, name) { return Gloss.getComponents(element)[name]; };

        Gloss.connect = function (node) {
        ...
        Gloss.disconnect = function (node) {
            for (var name in node[DATA]) {
    ...
    var getComponentName = memoize(function (attribute) {
        return startsWith(attribute, 'gls-') || startsWith(attribute, 'data-gls-')
            ? camelize(attribute.replace('data-gls-', '').replace('gls-', ''))
            : false;
    });

    var Gloss = function (options) {
        this._init(options);
    };
    ...
    function Core (Gloss) {
        ...
        on(document, 'animationstart', function (ref) {
            ...
            if ((css(target, 'animationName') || '').match(/^gls-.*(left|right)/)) {
    ...
    function boot (Gloss) {
        ...
        var connect = Gloss.connect;
        var disconnect = Gloss.disconnect;
        ...
        function applyAttributeMutation(ref) {
            ...
            var component = Gloss.getComponent(target, name);
    ...
    var drop = {
        ...
        computed: {
            ...
            clsDrop: function(ref) {
                ...
                return clsDrop || ("gls-" + (this.$options.name));
            },
    ...
    var Slidenav = {
        ...
        computed: {
            ...
            icon: function(ref, $el) {
                ...
                return hasClass($el, 'gls-slidenav-large')
                    ? (icon + "-large")
                    : icon;
            }
    ...
    var Search = {
        ...
        computed: {
            ...
            icon: function(ref, $el) {
                ...
                return hasClass($el, 'gls-search-icon') && parents($el, '.gls-search-large').length
                    ? 'search-large'
                    : parents($el, '.gls-search-navbar').length
                        ? 'search-navbar'
                        : icon;
            }
    ...
    var Close = {
        ...
        computed: {
            ...
            icon: function() {
                return ("close-" + (hasClass(this.$el, 'gls-close-large') ? 'large' : 'icon'));
            }
    ...
    function install$3(Gloss) {
        Gloss.icon.add = function (name, svg) {
            var obj;
            ...
            if (Gloss._initialized) {
                apply$1(document.body, function (el) { return each(Gloss.getComponents(el), function (cmp) {
        ...
        modal.alert = function (message, options) {
            return openDialog(
                function (ref) {
                    ...
                    return ("<div class=\"gls-modal-body\">" + (isString(message) ? message : html(message)) + "</div> <div class=\"gls-modal-footer gls-text-right\"> <button class=\"gls-button gls-button-primary gls-modal-close\" autofocus>" + (labels.ok) + "</button> </div>");
            },
                options,
        ...
        modal.confirm = function (message, options) {
            return openDialog(
                function (ref) {
                    ...
                    return ("<form> <div class=\"gls-modal-body\">" + (isString(message) ? message : html(message)) + "</div> <div class=\"gls-modal-footer gls-text-right\"> <button class=\"gls-button gls-button-default gls-modal-close\" type=\"button\">" + (labels.cancel) + "</button> <button class=\"gls-button gls-button-primary\" autofocus>" + (labels.ok) + "</button> </div> </form>");
            },
                options,
    ...
    var navItem = '.gls-navbar-nav > li > a, .gls-navbar-item, .gls-navbar-toggle';
...

            {
                ...
                handler: function(_, ref) {
                    ...
                    if (this.dropbarMode === 'slide') {
                        addClass(this.dropbar, 'gls-navbar-dropbar-slide');
                    }
...

            {
                ...
                read: function(data$1) {
                    ...
                    this.elements.forEach(function (el) {
                        ...
                        if (!el[stateKey]) {
                            el[stateKey] = {cls: data(el, 'gls-scrollspy-class') || this$1$1.cls};
                        }
        ...
        methods: {
            ...
            toggle: function(el, inview) {
                ...
                if (/\bgls-animation-/.test(state.cls)) {
                    state.off = once(el, 'animationcancel animationend', function () { return removeClasses(el, 'gls-animation-\\w*'); }
                    );
                }
        ...
        computed: {
            ...
            toggles: {
                ...
                get: function(ref, $el) {
                    ...
                    return $$(toggle, $el).filter(function (el) { return !matches(el, '.gls-disabled *, .gls-disabled, [disabled]'); });
                },
    ...
    var tab = {
        ...
        connected: function() {
            ...
            var cls = hasClass(this.$el, 'gls-tab-left')
                ? 'gls-tab-left'
                : hasClass(this.$el, 'gls-tab-right')
                    ? 'gls-tab-right'
                    : false;
    ...
    // register components
    each(components$1, function (component, name) { return Gloss.component(name, component); }
    );
    ...
    var clsLeave = 'gls-transition-leave';
    var clsEnter = 'gls-transition-enter';
    ...
    function fade(action, target, duration, stagger) {
        ...
        var enterFn = wrapIndexFn(function () {
            ...
            // Ensure Gloss updates have propagated
        ...
        methods: {
            ...
            updateNav: function() {
                ...
                this.navItems.forEach(function (el) {
                    ...
                    toggleClass(el, 'gls-invisible', this$1$1.finite && (cmd === 'previous' && i === 0 || cmd === 'next' && i >= this$1$1.maxIndex));
                });
    ...
    function install$1(Gloss, Lightbox) {
        ...
        assign(
            Lightbox.props,
            Gloss.component('lightboxPanel').options.props
        );
    ...
    function install(Gloss) {
        Gloss.notification.closeAll = function (group, immediate) {
            apply$1(document.body, function (el) {
                var notification = Gloss.getComponent(el, 'notification');
    ...
    each(components, function (component, name) { return Gloss.component(name, component); }
    );
